...
(no root `elementor-reference.json` in this repo as of this writing).
...
Do not use the retired blue palette (`#3D73FF`) for Elementor JSON. Use the
live theme tokens below and the expanded token notes in `elementor/README.md`.
...
No root `elementor-reference.json` exists in this repo as of <date>.
