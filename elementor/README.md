...
Neither file was produced from a real Elementor export. There is no `elementor-reference.json` anywhere in this repo or in `dataenglab-generator/references/`, and I could not reach `https://dataenglab.com` from this environment (WebFetch got HTTP 403; `curl` failed with a TLS revocation-check error — this sandbox has no working path to the live internet). Everything here was inferred from:
...
`color:#0B5A50`, the `--teal-ink` token this same corpus's
[`references/dataenglab-live-theme.md`](../references/dataenglab-live-theme.md)
already documents as "text on teal tint" — now ≈8:1 on tint and ≈8.1:1 on
